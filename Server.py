...
# TODO: Finish implementation of users with passwords:
...
class ThreadedTCPServer(socketserver.TCPServer):
    ...
    def initialize_db(self):
        ...
        query += " (username TEXT PRIMARY KEY, password TEXT, moderator INT);"
        self.db_cursor.execute(query)  # Run the query
    ...
    def register_username(self, username, password):
        with self.lock:
            query = "INSERT INTO users (username, password, moderator) VALUES (?, ?, ?);"
            self.db_cursor.execute(query, (username, password, 0))
            self.db_con.commit()
            self.users.append([username, password, 0])
